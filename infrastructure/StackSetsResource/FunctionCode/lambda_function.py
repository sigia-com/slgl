...
def update_stacks(set_region, set_name, accts, regions, param_overrides, ops_prefs):
    ...
    logger.info("StackSetName: {}, Accounts: {}, Regions: {}, ParameterOverrides: {}".format(
        set_name, accts, regions, param_overrides))
...
def update_stack_set(set_region, set_id, set_description, set_template,
                     set_parameters, set_capabilities, set_tags, set_ops_prefs, set_admin_role_arn, set_exec_role_name):
    # Set up for retries
    sleep_time = 15
    retries = 60
    this_try = 0
    ...
    while True:
        try:
            response = client.update_stack_set(
                ...
                Tags=set_tags,
                OperationPreferences=set_ops_prefs,
...
def create(event, context):
    ...
    # Debug
    logger.info("StackSetName: {}".format(set_name))
    logger.info("Description: {}".format(set_description))
    logger.info("TemplateURL: {}".format(set_template))
    logger.info("Parameters: {}".format(set_parameters))
    logger.info("Capabilities: {}".format(set_capabilities))
    logger.info("Tags: {}".format(set_tags))
    logger.info("AdministrationRoleARN: {}".format(set_admin_role_arn))
    logger.info("ExecutionRoleName: {}".format(set_exec_role_name))
...
def update(event, context):
    ...
    if 'OperationPreferences' in event['ResourceProperties']:
        ...
        set_ops_prefs = {}
    logger.info("OperationPreferences: {}".format(set_ops_prefs))
    ...
    if stack_set_needs_update:
        ...
        if 'OperationPreferences' in event['ResourceProperties']:
            set_ops_prefs = convert_ops_prefs(event['ResourceProperties']['OperationPreferences'])
        elif 'OperationPreferences' in event['OldResourceProperties']:
            set_ops_prefs = convert_ops_prefs(event['OldResourceProperties']['OperationPreferences'])
        else:
            set_ops_prefs = {}
        logger.info("OperationPreferences: {}".format(set_ops_prefs))
        ...
        if 'AdministrationRoleARN' in event['ResourceProperties']:
            set_admin_role_arn = event['ResourceProperties']['AdministrationRoleARN']
        elif 'AdministrationRoleARN' in event['OldResourceProperties']:
            set_admin_role_arn = event['OldResourceProperties']['AdministrationRoleARN']
        else:
            set_admin_role_arn = ''
        logger.info("AdministrationRoleARN: {}".format(set_admin_role_arn))
        ...
        if 'ExecutionRoleName' in event['ResourceProperties']:
            set_exec_role_name = event['ResourceProperties']['ExecutionRoleName']
        elif 'ExecutionRoleName' in event['OldResourceProperties']:
            set_exec_role_name = event['OldResourceProperties']['ExecutionRoleName']
        else:
            set_exec_role_name = ''
        logger.info("ExecutionRoleName: {}".format(set_exec_role_name))
        ...
        # Debug
        logger.info("OldResourceProperties: {}".format(event['OldResourceProperties']))
        logger.info("ResourceProperties: {}".format(event['ResourceProperties']))
